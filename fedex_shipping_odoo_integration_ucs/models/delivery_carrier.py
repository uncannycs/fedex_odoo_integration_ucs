...
class DeliveryCarrier(models.Model):
    ...
    fdx_rate_request_type = fields.Selection([('LIST', 'LIST'),
                                          ...
                                          default='ACCOUNT',
                                          help="LIST - Retrieves FedEx published list rates along with account-specific rates. PREFERRED - Retrieves rates in preferred currency. ACCOUNT - Retrieves account-specific rates (Default). INCENTIVE - One-time promotional discount. Contact your FedEx representative for details.")
    ...
    fdx_insurance = fields.Boolean(string="Shipment Insurance",
                                     ...
                                     default=False)
    fdx_smartpost_hub = fields.Selection([('5015', 'NOMA Northborough - 5015'),
                                     ...
                                     ('5983', 'SEWA Seattle - 5983')],string="SmartPost Hub")
    fdx_smartpost_indicia = fields.Selection([('MEDIA_MAIL', 'MEDIA_MAIL'),
                                      ('PARCEL_SELECT', 'PARCEL_SELECT'),
                                      ('PRESORTED_BOUND_PRINTED_MATTER', 'PRESORTED_BOUND_PRINTED_MATTER'),
                                      ('PRESORTED_STANDARD', 'PRESORTED_STANDARD')],string="SmartPost Indicia")
    ...
    def get_fdx_address(self, address_id):
        address = {
            ...
            "residential": "true" if self.fdx_shipping_service in ['GROUND_HOME_DELIVERY','SMART_POST'] else "false"
        }
    ...
    def fdx_delivery_rate_shipment(self, order):
        ...
        try:
            ...
            if self.fdx_shipping_service == 'SMART_POST':
                request_data.get("requestedShipment").update({"smartPostInfoDetail": {
                    "indicia": self.fdx_smartpost_indicia,
                    "hubId": self.fdx_smartpost_hub
                }}),
